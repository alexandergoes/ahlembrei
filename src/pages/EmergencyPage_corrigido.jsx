        ...
        >
          ...
          {notifySent ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
              ...
              <p className="text-green-800 font-medium">Família notificada!</p>
              <p className="text-green-600 text-sm mt-1">Os telefones dos contatos foram liberados abaixo.</p>
            </div>
          ) : (
            <Button
                ...
                <div key={contact.id} className="border border-gray-200 rounded-xl p-4">
                  ...
                  {phoneRevealed ? (
                    ...
                    <p className="text-sm text-gray-400 text-center py-2">
                      🔒 Clique em "Notificar Família Agora" para liberar os telefones
                    </p>
                  )}
                </div>
              ))}
...
const EmergencyPage = () => {
  ...
  const handleNotifyFamily = async () => {
    ...
    try {
      ...
      const mapsLink = `https://www.google.com/maps?q=${latitude},${longitude}`;
      const message = `Olá, estou ajudando ${profile.full_name} em uma emergência. Localização: ${mapsLink}`;
      // CORRIGIDO: window.location.href em vez de window.open para evitar bloqueio de popup
